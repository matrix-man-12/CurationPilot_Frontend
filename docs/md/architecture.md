...
6. **Multi-Sensory HITL Alerts** — Desktop alerts are triggered natively using the HTML5 Notification API. In addition, to support platforms without push support (like standard iOS Safari) or silent desktop scenarios, the system fires a synthetic electronic audio beep (via the Web Audio API) and flashes the document tab title (toggling between `"🔴 Action Required!"` and `"⚠️ CurationPilot"`). These alerts trigger only once per HITL step, and clear immediately when focus or click returns to the viewport.
